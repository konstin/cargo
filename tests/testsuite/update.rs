...
/// Regression test for https://github.com/rust-lang/cargo/issues/5529: Don't change the version
/// of `shared` unnecessarily.
///
/// ```
/// foo
/// ├── shared =0.1.0
...
/// ```
///
/// When `c` gets updated, the version of `shared` that `b` uses should not be updated, to avoid
/// churning the lockfile between different compatible versions.
#[cargo_test]
fn update_preserves_unrelated_dependency_choices() {
    ...
    Package::new("b", "1.0.0")
        .dep("shared", ">=0.1, <0.3")
        .publish();
    Package::new("c", "1.0.0").dep("b", "=1.0.0").publish();
...
[UPDATING] `dummy-registry` index
[LOCKING] 4 packages to latest compatible versions
[ADDING] shared v0.1.0 (available: v0.3.0)
...
"##]],
    ...
    p.cargo("update -p c --precise 1.0.1")
        .with_stderr_data(str![[r#"
[UPDATING] `dummy-registry` index
